...
if($DT['cloud_express'] && DT_CLOUD_UID && DT_CLOUD_KEY) {
	$cache = 0;
	$cid = DT_ROOT.'/file/cloud/express/'.urlencode($_com).'/'.substr($no, -6, 3).'/'.substr($no, -3).'/'.$no.'.php';
	if(is_file($cid)) {
	...
	if($arr['data']) {
		...
		if($htm) $htm .= '<div>- 以上数据由<a href="'.DT_PATH.'api/redirect.php?url=http://www.ickd.cn/?from=DESTOON" class="t" target="_blank">爱查快递</a>提供，如有疑问请到<a href="'.DT_PATH.'api/express.php?action=home&e='.urlencode($com).'&n='.$no.'" class="t" target="_blank">快递公司官网</a>查询</div>';
	}
}
